...
function startColumnRename(column, titleEl) {
    ...
    async function saveRename() {
        ...
        newTitle.dataset.columnId = column.id;
        newTitle.textContent = `${column.name} (${taskCount})`;
        newTitle.addEventListener('dblclick', () => startColumnRename(column, newTitle));
...
async function handleDeleteColumn(column) {
    ...
    const taskCount = column.tasks ? column.tasks.length : 0;
    let confirmMsg = `Delete "${column.name}"?`;
    if (taskCount > 0) {
        confirmMsg = `Delete "${column.name}"?\n\n⚠️ This will also delete ${taskCount} task${taskCount === 1 ? '' : 's'} in this column.`;
    }
...
document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
        [taskModal, columnModal, editTaskModal].forEach(closeModal);
    }
});
